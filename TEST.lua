...
MainFrame.Position = UDim2.new(0, -300, 0.5, -250)
...
local function prévoirButton(parent, text, color, callback)
...
local antikickButton, antikickUpdateStatus = createButton(MainFrame, "Anti-Kick", Color3.fromRGB(40, 40, 40), function(updateStatus)
    antikickEnabled = not antikickEnabled
    if antikickEnabled then
        loadstring(game:HttpGet("https://raw.githubusercontent.com/Exunys/Anti-Kick/main/Anti-Kick.lua"))()
    end
    updateStatus(antikickEnabled)
end)

-- Fitur Dex
local dexButton, dexUpdateStatus = createButton(MainFrame, "Dex", Color3.fromRGB(40, 40, 40), function(updateStatus)
    loadstring(game:HttpGet("https://raw.githubusercontent.com/peyton2461/Dex/master/misc/release.lua"))()
    updateStatus(true)
end)

-- Fitur No Prompts
local noPromptsEnabled = false
local noPromptsButton, noPromptsUpdateStatus = createButton(MainFrame, "No Prompts", Color3.fromRGB(40, 40, 40), function(updateStatus)
    noPromptsEnabled = not noPromptsEnabled
    if noPromptsEnabled then
        for _, coreGuiType in pairs(Enum.CoreGuiType:GetEnumItems()) do
            game.StarterGui:SetCoreGuiEnabled(coreGuiType, false)
        end
    else
        for _, coreGuiType in pairs(Enum.CoreGuiType:GetEnumItems()) do
            game.StarterGui:SetCoreGuiEnabled(coreGuiType, true)
        end
    end
    updateStatus(noPromptsEnabled)
end)

-- Fitur Anti-Lag
local antilagEnabled = false
local antilagButton, antilagUpdateStatus = createButton(MainFrame, "Anti-Lag", Color3.fromRGB(40, 40, 40), function(updateStatus)
    antilagEnabled = not antilagEnabled
    if antilagEnabled then
        -- Mengurangi lag dengan menurunkan kualitas render dan menghapus efek berlebihan
        ...
        print("Anti-Lag Dinonaktifkan")
    end
    updateStatus(antilagEnabled)
end)